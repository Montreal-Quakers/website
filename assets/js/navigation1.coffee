...
y = document.getElementById("myNavbar");
...
observer1 = new IntersectionObserver(callback1);
...
if (mq.matches) {
  observer2 = new IntersectionObserver(callback2);
  ...
  function responsiveClick(id) {
   ...
   truth = x.previousElementSibling.firstChild.classList.contains('fa-caret-right')
   if (truth) {
...
function flipIcon () {
  i = document.getElementById("flippy");
  i2 = document.getElementById("flippy2");
  ...
  if (i2.classList.contains("searchy")) {
    ...
    newstring = parsedUrl.pathname + "?q=";
    newurl = new URL(i2.href);
    param = parsedUrl.searchParams.get("q");
...
unf = document.getElementById("unfold");
sub = document.getElementById("searchsubmit");
